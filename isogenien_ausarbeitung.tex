...
\begin{proof}
  \begin{enumerate}[label=\roman*)]
  \item Für \emph{i)} müssen wir nur die Aussage aus \autoref{sepgrad}\,ii)
    (die Behauptung in \emph{i)} von oben gilt für alle außer endlich
    viele Punkte in $E_2$) erweitern. Dazu zeigen wir, dass
    $\#\phi^{-1}(Q)$ für alle Punkte $Q\in E_2$ gleich ist.
