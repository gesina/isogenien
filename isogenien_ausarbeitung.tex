...
\begin{Satz}
  ...
  \begin{proof}
    ...
    $\algK(E)/\algK(E)^\Psi$ ist eine Galoiserweiterung und wir
    erhalten dank der Kategorienäquivalenz aus
    \autoref{kategorienaequivalenz} eine eindeutige glatte Kurve
    $C/\algK$ zu $\algK(E)^\Psi$ und einen endlichen Morphismus
    $\phi\colon E\to C$ zur Erweiterung $\algK(E)^\Psi\to\algK(E)$
    mit $\phi^*(\algK(C))=\algK(E)^\Psi$.
